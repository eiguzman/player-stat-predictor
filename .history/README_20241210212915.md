...
- **Application for Regression:** Initial decision was to actually use the categorical datas `Tm` and `Pos` because we reasoned team environment/position would attribute to a difference. Never ended up using it.
- 
#### **Features to Per Game stats**
- In order to predict PPG, we wanted per game versions of the statistics that were none percentages.

## Model 1

**Feature Selection**From our discussion we decided to use these selections `TRB_per_game`, `AST_per_game`, `MP_per_game`, `3PA_per_game`, `2PA_per_game`, `FTA_per_game`,`Age`, `FGA_per_game` and `OBPM`


#### **Results and configuration with our model**
- Our Resulting MSE from our first model was shockingly low
```python
Training MSE: 0.52
Testing MSE: 0.50
```
In the beginning we had chosen a linear model as a low complexity baseline so we were sure under fitting was happening; however the low score for both training and testing MSE was not indications of such.

The stable Cross-Validation score also indicated our model was not overfitting 
```python 
Coefficients: [-4.2614373  -1.42741702  0.23164253 -0.79862453 -7.9158575  10.2326818
 ...
 42.98666333]
```
After looking at the Correlation of matrix features figure, we realized that there was a very strong correlation between ppg and fga. The model coefficients indicated that FGA was hyperdominating every other feature. FGA is also comprised of 3pa and 2pa combined (which are in the model already) we decided to drop FGA.

After checking our model against
```python
Training MSE for no FGA: 1.65
Testing MSE for no FGA: 1.59
```
